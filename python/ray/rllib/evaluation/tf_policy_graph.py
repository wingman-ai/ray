...
class TFPolicyGraph(PolicyGraph):
    ...
    def _build_compute_actions(self,
        ...
        language_inputs = self.model.language_inputs if hasattr(self.model, 'language_inputs') else self._values

        fetches = builder.add_fetches([self._sampler] + self._state_outputs +
                                      [self.extra_compute_action_fetches()] + [self._values] + [language_inputs])
